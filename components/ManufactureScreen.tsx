...
export default function ManufactureScreen() {
  ...
  return (
    <div className="relative isolate min-h-screen w-full overflow-hidden bg-[#120b16] text-[#fff5de]">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(255,239,185,0.12),transparent_24%),linear-gradient(180deg,rgba(22,12,27,0.12)_0%,rgba(23,12,28,0.42)_54%,rgba(13,7,16,0.76)_100%)]" />
      ...
      <div className="relative z-10 flex min-h-screen flex-col px-3 pb-4 pt-4 sm:px-4 sm:pb-5 sm:pt-5 lg:px-8 lg:pb-6 lg:pt-6">
        <div className="pointer-events-none mb-2 flex justify-center sm:mb-4">
          <div className="pointer-events-auto flex max-w-[min(88vw,42rem)] flex-col items-center gap-2 sm:gap-3">
            <div className="rounded-[1.75rem] border-[3px] border-[#f0d4a2] bg-[#fff3df]/92 px-4 py-3 text-center text-[#41251d] shadow-[0_18px_40px_rgba(32,16,24,0.24)] backdrop-blur-sm sm:rounded-[2rem] sm:px-6 sm:py-4">
              <p className="mb-1 text-[0.58rem] font-bold uppercase tracking-[0.42em] text-[#9c6a48] sm:text-[0.65rem]">
                spell prompt
              </p>
            ...
            <button
              ...
              className="rounded-full border border-white/35 bg-[#241621]/72 px-4 py-1.5 text-[0.68rem] font-bold tracking-[0.18em] text-[#ffe4b0] shadow-[0_10px_18px_rgba(18,8,18,0.32)] transition hover:-translate-y-0.5 hover:bg-[#332032]/78 hover:text-white sm:text-[0.75rem]"
            >
              문장 다시 뽑기
            </button>
          </div>
        ...
        <div className="grid flex-1 grid-cols-[minmax(5.85rem,1fr)_minmax(9.5rem,11rem)_minmax(5.85rem,1fr)] items-center gap-2 sm:grid-cols-[minmax(7.4rem,1fr)_minmax(12rem,15.5rem)_minmax(7.4rem,1fr)] sm:gap-3 md:grid-cols-[minmax(9.5rem,1fr)_minmax(15rem,20rem)_minmax(9.5rem,1fr)] md:gap-4 lg:grid-cols-[minmax(11rem,17rem)_minmax(18rem,27rem)_minmax(11rem,17rem)] lg:gap-8">
          <aside className="flex flex-col items-end justify-center gap-2 sm:gap-3 lg:gap-4">
            {leftScents.map((scent) => (
              <ScentButton
                ...
                disabled={totalSelections >= 10}
              />
            ))}
          </aside>

          <section className="relative flex h-full items-center justify-center">
            <div className="pointer-events-none absolute inset-x-[9%] bottom-[10%] h-16 rounded-full bg-[#12090f]/75 blur-2xl sm:h-20 lg:bottom-[11%] lg:h-28" />

            <div className="absolute left-1/2 top-[6%] z-30 w-[10.5rem] -translate-x-1/2 sm:w-[12rem] md:left-auto md:right-[-12%] md:top-[18%] md:w-[13rem] md:translate-x-0 lg:right-[-10%] lg:w-[14.5rem]">
              <div className="rounded-[1.6rem] border-[2px] border-[#f2ca84] bg-[#2b1825]/78 px-3 py-3 shadow-[0_20px_36px_rgba(17,8,17,0.36)] backdrop-blur-md sm:px-4 lg:rounded-[1.9rem] lg:px-5">
                <p className="text-[0.56rem] font-bold uppercase tracking-[0.34em] text-[#f3d18a] sm:text-[0.6rem]">
                  brew status
                </p>

                <div className="mt-1 flex items-end gap-2">
                  <span className="text-[1.7rem] font-black leading-none sm:text-[2rem] lg:text-[2.35rem]">
                    {totalSelections}
                  </span>
                ...
                <div className="mt-2 h-2 rounded-full bg-white/12">
                  <div
                    className="h-full rounded-full bg-[linear-gradient(90deg,#ffe195_0%,#ff9d5c_55%,#c66cff_100%)] shadow-[0_0_16px_rgba(255,199,126,0.55)] transition-all duration-300"
                    style={{ width: `${Math.min(totalSelections, 10) * 10}%` }}
                  />
                </div>

                <p className="mt-2 text-[0.64rem] leading-relaxed text-[#f7dfbf] sm:text-[0.69rem] lg:text-[0.75rem]">
            ...
            <div className="cauldron-float relative flex aspect-[4/5] w-full max-w-[26rem] items-end justify-center">
              ...
              {['left-[19%] top-[47%]', 'left-1/2 top-[55%] -translate-x-1/2', 'right-[19%] top-[47%]'].map(
                (position, index) => (
                  <div
                    key={position}
          ...
          <aside className="flex flex-col items-start justify-center gap-2 sm:gap-3 lg:gap-4">
        ...
        <div className="pointer-events-none mt-1 flex justify-center sm:mt-3">
          <div className="rounded-full border border-white/18 bg-[#1c1221]/68 px-4 py-2 text-center text-[0.68rem] text-[#f6e7cf] shadow-[0_10px_18px_rgba(10,4,12,0.2)] backdrop-blur-md sm:text-[0.78rem]">
            재료를 10번 고르면 향수 결과 화면으로 이어집니다.
          </div>
        </div>
      </div>
    </div>
